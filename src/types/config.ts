...
export interface ConfigurableItem {
  ...
  isArchived?: boolean; // Hidden but data preserved
}

// Status configuration
export interface StatusConfig extends ConfigurableItem {
  type: 'active' | 'completed' | 'cancelled'; // For logic grouping
  showInBoard: boolean;
  countAsProgress: boolean; // Whether tasks in this status count toward completion
}

// Area/Category configuration
...
export interface WorkspaceConfig {
  ...
  defaults: {
    status: string; // ID of default status
    priority: string; // ID of default priority
    cadence: string; // ID of default cadence
    confidence: string; // ID of default confidence
    taskStatus: string; // ID of default task status
  };
  ...
  board: {
    ...
    cardDisplayFields: string[]; // Which fields to show on cards
  };
...
// Custom field support (for future)
...
export const DEFAULT_AREAS: AreaConfig[] = [
  { id: 'area-faith', name: 'Faith', color: 'violet', order: 1, description: 'Spiritual growth and practices' },
  { id: 'area-family', name: 'Family', color: 'pink', order: 2, description: 'Family relationships and activities' },
  { id: 'area-learning', name: 'Learning', color: 'sky', order: 3, description: 'Education and skill development' },
  { id: 'area-career', name: 'Career', color: 'emerald', order: 4, description: 'Professional growth' },
  { id: 'area-finance', name: 'Finance', color: 'amber', order: 5, description: 'Financial goals and management' },
  { id: 'area-health', name: 'Health', color: 'red', order: 6, description: 'Physical and mental wellbeing' },
  { id: 'area-personal', name: 'Personal', color: 'purple', order: 7, description: 'Personal development' },
  { id: 'area-business', name: 'Business', color: 'blue', order: 8, description: 'Business ventures' },
];
...
export const DEFAULT_TASK_STATUSES: TaskStatusConfig[] = [
  { id: 'task-backlog', name: 'Backlog', color: 'slate', order: 1, type: 'pending', countAsProgress: false },
  { id: 'task-next', name: 'Next Action', color: 'blue', order: 2, type: 'active', countAsProgress: false },
  { id: 'task-done', name: 'Done', color: 'emerald', order: 3, type: 'completed', countAsProgress: true },
];

export const DEFAULT_WORKSPACE_CONFIG: Omit<WorkspaceConfig, 'id' | 'workspaceId' | 'createdAt' | 'updatedAt'> = {
...
export const COLOR_PRESETS = [
  ...
  { name: 'Lime', value: 'lime', bg: 'bg-lime-100', text: 'text-lime-700', border: 'border-lime-200' },
  ...
  { name: 'Cyan', value: 'cyan', bg: 'bg-cyan-100', text: 'text-cyan-700', border: 'border-cyan-200' },
  ...
  { name: 'Fuchsia', value: 'fuchsia', bg: 'bg-fuchsia-100', text: 'text-fuchsia-700', border: 'border-fuchsia-200' },
  { name: 'Pink', value: 'pink', bg: 'bg-pink-100', text: 'text-pink-700', border: 'border-pink-200' },
  { name: 'Rose', value: 'rose', bg: 'bg-rose-100', text: 'text-rose-700', border: 'border-rose-200' },
];

// Helper to get color classes from color value
export function getColorClasses(colorValue: string): { bg: string; text: string; border: string } {
  ...
  // Fallback for custom colors or unknown values
  return {
