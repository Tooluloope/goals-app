...
export default function AreaProjectsPage() {
  ...
  const areaStats = useMemo(() => {
    ...
    const pendingTasks: { task: Project['tasks'][0]; project: Project }[] = [];
    areaProjects.forEach(p => {
      p.tasks
  ...
  }, [areaProjects, currentWorkspace, getStatusById, cadences, doneTaskStatusIds, nextActionStatusId]);

  const areaColors = area ? getColorClasses(area.color) : { bg: 'bg-slate-100', text: 'text-slate-700' };
  const gradient = area ? AREA_GRADIENTS[area.color] || 'from-slate-500 to-gray-600' : 'from-slate-500 to-gray-600';
    ...
    return (
      <AppLayout title="Projects">
        <div className="flex flex-col items-center justify-center min-h-[60vh] p-4">
          ...
          <p className="text-muted-foreground mb-4">This area doesn&apos;t exist or has been removed.</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {areaProjects.length === 0 ? (
                ...
                <Accordion type="multiple" defaultValue={areaProjects.map(p => p.id)} className="space-y-4">
...
function ProjectCard({
  ...
  const priority = currentWorkspace ? getPriorityById(currentWorkspace.id, project.priorityId) : null;
  const taskStatuses = currentWorkspace ? getTaskStatusesForWorkspace(currentWorkspace.id) : [];
  const nextActionStatusId = taskStatuses.find(s => s.name === 'Next Action')?.id || 'task-next';
  ...
  const statusColors = status ? getColorClasses(status.color) : { bg: 'bg-slate-100', text: 'text-slate-700' };
  const priorityColors = priority ? getColorClasses(priority.color) : { bg: 'bg-slate-50', text: 'text-slate-600' };

  const nextActions = project.tasks.filter(t => t.statusId === nextActionStatusId);
  const latestReview = project.reviewNotes[project.reviewNotes.length - 1];
