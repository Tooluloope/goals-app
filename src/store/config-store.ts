...
interface ConfigState {
  ...
  updatePriority: (workspaceId: string, priorityId: string, updates: Partial<PriorityConfig>) => void;
  ...
  updateTaskStatus: (workspaceId: string, statusId: string, updates: Partial<TaskStatusConfig>) => void;
  ...
  updateDashboardSettings: (workspaceId: string, dashboard: Partial<WorkspaceConfig['dashboard']>) => void;
  updateNotificationSettings: (workspaceId: string, notifications: Partial<WorkspaceConfig['notifications']>) => void;
  ...
  persist(
    (set, get) => ({
      ...
      reorderAreas: (workspaceId, areaIds) => {
        const config = get().getConfig(workspaceId);
        const reordered = areaIds.map((id, index) => {
      ...
      updateCadence: (workspaceId, cadenceId, updates) => {
        ...
        set((state) => ({
          configs: {
            ...
            [workspaceId]: {
              ...config,
              cadences: config.cadences.map((c) =>
                c.id === cadenceId ? { ...c, ...updates } : c
              ),
              updatedAt: new Date().toISOString(),
            },
          },
