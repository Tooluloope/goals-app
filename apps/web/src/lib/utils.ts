 ...
 * Handles the fact that JSON serialization converts Date objects to ISO strings,
 * but TypeScript types may still indicate Date.
 */
...
/**
 * Converts a Date or ISO string to an ISO string.
 * Useful when you need a string representation for date-fns functions.
 */
...
export function calculateProjectProgress(
  ...
  const requirementsDone = requirements.filter((r) => r.completed).length;
  const definitionDone = definitionOfDone.filter((d) => d.completed).length;
  const tasksDone = tasks.filter((t) => completedTaskStatusIds.includes(t.statusId)).length;
...
export function isReviewDue(
  ...
  const lastReview = toDate(project.lastReviewDate);
...
export function isProjectStale(
  ...
  const lastUpdate = toDate(project.updatedAt);
  const today = new Date();
...
export function isTaskOverdue(dueDate: Date | string | null | undefined): boolean {
  if (!dueDate) return false;
  const due = toDate(dueDate);
  const today = new Date();
...
export function getDaysUntilDeadline(targetDate: Date | string): number {
  const target = toDate(targetDate);
  const today = new Date();
...
export function getNextReviewDate(
  lastReviewDate: Date | string | null | undefined,
  cadenceId: string,
  cadences: CadenceConfig[] = DEFAULT_CADENCES
): Date {
  const base = lastReviewDate ? toDate(lastReviewDate) : new Date();
...
export function getDaysSinceLastReview(
  lastReviewDate: Date | string | null | undefined
): number | null {
  if (!lastReviewDate) return null;
  return differenceInDays(new Date(), toDate(lastReviewDate));
}
...
export function sortProjectsByUrgency(
  ...
  return [...projects].sort((a, b) => {
    ...
    // Then by target date
    return differenceInDays(toDate(a.targetDate), toDate(b.targetDate));
  });
}
