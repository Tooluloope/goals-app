...
const EVENT_COLORS: Record<
  EventType,
  { bg: string; bgDark: string; text: string; textDark: string; border: string; dot: string }
> = {
  deadline: {
    bg: 'bg-red-100',
...
export default function CalendarPage() {
  ...
  const priorityEvent =
    selectedDateEvents.find((e) => e.urgency === 'critical' || e.urgency === 'warning') ||
    ...
    <AppLayout title="Calendar">
      <div className="flex flex-col h-full overflow-hidden">
          ...
          <div className="max-w-[1600px] mx-auto px-4 sm:px-6 py-6 sm:py-8">
            ...
            <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
              <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl p-4 hover:bg-white/15 transition-all cursor-pointer group">
                ...
                <p className="mt-1 text-3xl font-bold group-hover:scale-105 transition-transform origin-left">
                  {monthStats.deadlines}
                </p>
              </div>
              <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl p-4 hover:bg-white/15 transition-all cursor-pointer group">
                <div className="flex items-center gap-2 text-indigo-200 text-xs font-semibold uppercase tracking-wider">
          ...
          <div className="h-full max-w-[1600px] mx-auto w-full flex flex-col lg:flex-row">
            ...
            <div className="w-full lg:w-[400px] flex flex-col gap-4 p-4 lg:p-6 lg:pl-2 shrink-0 overflow-y-auto">
                      ...
                      {priorityEvent && (
                        <div
                          className={cn(
                            ...
                            priorityEvent.urgency === 'critical' &&
                              'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-800/30',
                            priorityEvent.urgency === 'warning' &&
                              'bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-800/30',
                            priorityEvent.urgency === 'normal' &&
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            <div
                              className={cn(
                                ...
                                EVENT_COLORS[priorityEvent.type].dot
                              )}
                            />
                            <span
                              className={cn(
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div
                                className={cn('p-2 rounded-lg', eventColors.bg, eventColors.bgDark)}
                              >
                                {event.type === 'task' && (
                                  <ListTodo
                                    className={cn(
                  ...
                  <CardContent>
                    <div className="relative pl-4 border-l border-border space-y-6">
                      {upcomingEvents.map((event) => {
                        const eventColors = EVENT_COLORS[event.type];
                        const daysUntil = differenceInDays(toDate(event.date), new Date());

                        return (
                          <button
                            key={event.id}
                            onClick={() => {
                              setSelectedCalendarDate(toDate(event.date));
                            ...
                            <div className="flex flex-col gap-1">
                              <div className="flex items-center justify-between">
                                <span className="text-xs font-medium text-muted-foreground">
                                  {format(toDate(event.date), 'd MMM')}
                                </span>
              ...
              <div className="bg-muted/50 rounded-xl p-5 shadow-inner">
                <h3 className="font-bold text-sm uppercase tracking-wide mb-3 opacity-70">
                  Quick Actions
                </h3>
                <div className="space-y-3">
                  <Button
        ...
        <div className="fixed bottom-20 md:bottom-8 right-6 z-50">
          <Button
            size="lg"
            className="h-14 w-14 p-0 rounded-full bg-gradient-to-r from-primary to-indigo-600 hover:from-indigo-500 hover:to-indigo-700 shadow-lg hover:shadow-primary/30 hover:scale-110 active:scale-95 transition-all text-white"
            onClick={() => setAddProjectModalOpen(true)}
          >
