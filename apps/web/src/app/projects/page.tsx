...
export default function ProjectsPage() {
    ...
    return areas
      .map((area) => {
        ...
        const pendingTasks = areaProjects.reduce((acc, p) => {
          return (
            acc + (p.tasks ?? []).filter((t) => !doneTaskStatusIds.includes(t.statusId)).length
          );
        }, 0);
        ...
        const overallPercentage =
          total > 0
            ? Math.round(
                areaProjects.reduce(
                  (acc, p) => acc + calculateProjectProgress(p, doneTaskStatusIds).percentage,
                  0
                ) / total
              )
            : 0;
  ...
  const overallStats = useMemo(() => {
    ...
    const overallProgress =
      totalProjects > 0
        ? Math.round(
            areaStats.reduce((acc, s) => acc + s.overallPercentage * s.total, 0) / totalProjects
          )
        : 0;
          ...
          <div className="container mx-auto px-4 py-12">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/70">Browse all your goals organized by life areas</p>
              </div>
            </div>
                    ...
                    >
                      ...
                      <div className={cn('bg-gradient-to-br p-6 text-white', gradient)}>
                        ...
                        <div className="mt-4">
                          ...
                          <Progress value={overallPercentage} className="mt-2 h-2 bg-white/20" />
                        </div>
                      </div>
