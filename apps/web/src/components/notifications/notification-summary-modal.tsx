...
import { AlertCircle, Calendar, Clock, Target, Bell, ArrowRight } from 'lucide-react';
...
export function NotificationSummaryModal() {
  ...
  const { getStatusesForWorkspace, getCadencesForWorkspace, getTaskStatusesForWorkspace } =
    useConfigStore();
  ...
  const upcomingDeadlines = doingProjects.filter((p) => isDeadlineApproaching(p.targetDate, 30));
  ...
  const hasItems =
    unreadNotifications.length > 0 || reviewsDue.length > 0 || upcomingDeadlines.length > 0;
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {unreadNotifications.slice(0, 3).map((notification) => (
                    <div key={notification.id} className="rounded-lg border bg-card p-3">
